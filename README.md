...
## What you get — 15 read-only tools
...
| `kyrodata_get_credit_balance` | Reports the credit balance and the limits of the API key making the request: credits left in the current cycle and when it resets, the daily credit and daily call ceilings of the key, and which tools the key reaches. |
...
| `kyrodata_list_trade_series` | Returns Brazil's monthly export or import series as raw rows — one row per month with value (USD FOB), volume (kg) and the implied price per kg — over a month window (YYYYMM; the default is the last 24 published months). The window is capped at 200 months, and a month with nothing published is absent rather than zero. |
| `kyrodata_resolve_comparison_window` | Resolves an equal-weight comparison window (like-for-like) for the trade data, anchored on the last fully published month. |
...
questions inside a session are not charged twice. `kyrodata_get_credit_balance` always reports
